...
class Client(ABC):
    ...
    async def socket_reader(self) -> None:
        """Receive websocket messages, reconnecting on transient drops."""
        backoff = 1.0
        while True:
            try:
                async with self.session.ws_connect(
                    self.ws_uri, ssl=self._ssl_validation,
                ) as websocket:
                    backoff = 1.0
                    async for message in websocket:
                        await self._process_socket_message(message)
    ...
    async def _process_socket_message(self, message: Any) -> None:
...
def _ssl_validation_mode(base_url: str) -> Union[bool, ssl.SSLContext]:
    ...
    parsed = urlparse(base_url)
    if parsed.netloc.endswith(".drw") or parsed.netloc.endswith(".drw.com"):
        # If drw-certifi is not installed and this is a DRW games host,
        # bypass ssl (common in local dev without drw-certifi wheel).
